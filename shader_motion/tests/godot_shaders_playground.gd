...
func hlsl_vector2_ternary(condition: Vector2, if_values: Vector2, else_values: Vector2) -> Vector2:
	return Vector2(
		if_values.x if bool(condition.x) else else_values.x, if_values.y if bool(condition.y) else else_values.y
	)


func hlsl_vector3_ternary(condition: Vector3, if_values: Vector3, else_values: Vector3) -> Vector3:
	return Vector3(
...
func hlsl_vector2_and(condition_a: Vector2i, condition_b: Vector2i) -> Vector2i:
	return Vector2i(condition_a.x & condition_b.x, condition_a.y & condition_b.y)


func hlsl_vector2_equal(vector_a: Vector2, vector_b: Vector2) -> Vector2i:
...
func _sm_gray_decoder_sum(state: Vector3) -> float:
...
func _sm_gray_decoder_add(state: Vector3, value: float, radix: int, continuation: bool = true):
...
func _fake_texture_sampler(texture: Texture2D, sampler: RDSamplerState, uv: Vector2, lod: int) -> Color:
...
func _sm_sample_tile(color_tile: ColorTile, texture: Texture2D, rect: Vector4, sample_gamma: bool):
...
func _sm_get_tile_rect(uv: Vector2):
	...
	return (vec4_tile + Vector4(0, 0, 1, 1)) / Vector4(tile_count.x, tile_count.y, tile_count.x, tile_count.y)
...
func _sm_sample_snorm(uv: Vector2):
	...
	var tex_tiling: Vector4 = Vector4(_main_tex_st.x, _main_tex_st.y, _main_tex_st.x, _main_tex_st.y)
	var tex_offset: Vector4 = Vector4(_main_tex_st.z, _main_tex_st.w, _main_tex_st.z, _main_tex_st.w)
	var scaled_rect: Vector4 = rect * tex_tiling
	var transformed_rect: Vector4 = scaled_rect + tex_offset
	print("[_sm_sample_snorm]\n\tscaled_rect : %s\n\ttransformed_rect : %s" % [str(scaled_rect), str(transformed_rect)])
